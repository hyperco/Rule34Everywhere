// ==UserScript==
// @name        Rule34Everwhere
// @namespace   http://hyco.me/
// @description Add Rule34 Hovers everywhere
// @include     *
// @version     1
...
var banned = (function() {
  ...
  // do not include numbers below 100
  for (c = 0; c < 100; c++) {
    res.push(c.toString());
  }
  return res;
...
// using an underscore makes everything smaller for serialization
var ME_PROP = "_";

SearchTree.prototype.insert = function(tags, parts, full) {
  if (parts.length === 0) {
    if (tags.hasOwnProperty(ME_PROP)) {
      alert("WTF TAGS HAS PROP" + full);
    } else {
      tags[ME_PROP] = full;
    }
  } else {
    if (!tags.hasOwnProperty(parts[0])) {
      tags[parts[0]] = {};
    }
    this.insert(tags[parts[0]], parts.slice(1), full);
  }
};

SearchTree.prototype.findMatchesIn = function(s) {
  var m, WORD_REGEX = /\b(\w+)\b/g, word, matches = [];

  var search_state = this.store, match_start = null, backtrack_index = null;

  while ((m = WORD_REGEX.exec(s)) !== null) {
    word = m[0].toLowerCase();

    // not in array
    if (!search_state.hasOwnProperty(word)) {
      if (match_start === null) { continue; }

      if (search_state.hasOwnProperty(ME_PROP)) {
        matches.push([match_start, m.index+m[0].length, search_state[ME_PROP]]);
      } else {
        WORD_REGEX.lastIndex = backtrack_index;
      }

      match_start = null;
      search_state = this.store;
      backtrack_index = null;
    ...
    if (match_start === null) {
      match_start = m.index;
      backtrack_index = match_start + m[0].length;
    }
    search_state = search_state[word];
  }

  if (match_start !== null && search_state.hasOwnProperty(ME_PROP)) {
    matches.push([match_start, s.length, search_state[ME_PROP]]);
  }
  return matches;
};

SearchTree.prototype.serialize = function() {
  return JSON.stringify(this.store);
};

function anonParseHtml(htm) {
  var p = new DOMParser();
  var htmldoc = p.parseFromString(htm, "text/html");
//  var serializer = new XMLSerializer();
//  var s = serializer.serializeToString(htmldoc);
//  return $($.parseXML(s));
  return $(htmldoc);
}
...
function loadRandImg(el) {
  var my_tag = jQuery.data(el, "cur_tag");
  var post_list = "http://rule34.paheal.net/post/list/" +my_tag + "/1";
  el.text("Loading "+my_tag +"...");

  function err(r) {
    /* in the case that another tag has been loaded. */
    if (jQuery.data(el, "cur_tag") !== my_tag) { return; }

    el.text("Error loading "+my_tag+". (Paheal down?)");
  }
  ...
  GM_xmlhttpRequest({
    ...
    url : post_list,
    headers : {"Referer": ""},
    timeout: 5000, // fuck paheal is slow jesus christ
    onload : function(resp) {
      /* in the case that another tag has been loaded. */
      if (jQuery.data(el, "cur_tag") !== my_tag) { return; }

      var html = anonParseHtml(resp.responseText);
      var imgs = html.find(".shm-thumb-link");
      var id = Math.floor(Math.random() * imgs.length);
      var thumbnail = $(imgs.get(id));
      var thumb_url = "http://rule34.paheal.net"+thumbnail.attr("href");
      var thumb_src = thumbnail.find("img").prop("src");

      // Load the image content with a null referer
      // so we don't leak any data
      // See http://stackoverflow.com/questions/8778863/downloading-an-image-using-xmlhttprequest-in-a-userscript/8781262#8781262
      GM_xmlhttpRequest({
        method: "GET",
        url : thumb_src,
        headers : {"Referer": ""},
        ...
        onload : function(resp2) {
          ...
          var newThumbnail = $("<a/>").css({
            'margin': '0px',
            'border': '0px',
            'padding': '0px',
          });
          // can not use thumbnail.attr("href") because jquery will autorebase
          newThumbnail.prop("href", thumb_url);

          var ctype = (/Content-Type:\s*(\S+)/i).exec(resp2.responseHeaders)[1];
          var newImg = $("<img/>").css({
            'margin': '0px',
            'border': '0px',
            'padding': '0px',
          ...
          var data_uri = "data:"+ctype+";base64,"+customBase64Encode(resp2.responseText);
          newImg.prop("src", data_uri);
          newThumbnail.append(newImg);
          el.html(newThumbnail);
        },
        onerror: err,
        ontimeout: err,
        onabort: err,
...
}), pahealContent = $("<div class='paheal_content'></div>").css({
  'position': 'absolute',
...
function hoverIn(ev) {
  var dt = $(this), saved_tag = dt.data("saved_tag");
  dt.wrap(newel);
  var wrapper = dt.parent();
  wrapper.append(pahealContent);
  wrapper.hover(function(ev) { }, function(ev) { $(this).replaceWith(dt);});
  pahealContent.text(saved_tag);
  jQuery.data(pahealContent, "cur_tag", saved_tag);
  loadRandImg(pahealContent);
}
...
function processTextNode(node, tags) {
  var nodetext = node.nodeValue, idx = 0, tag, post_list, m, matches, i;

  if (WHITESPACE.test(nodetext)) { return false; }

  matches = tags.findMatchesIn(nodetext);

  if (matches.length === 0) { return false; }

  for (i = 0; i < matches.length; i++) {
    m = matches[i];
    node = node.splitText(m[0] - idx);
    idx = m[0];

    tag = encodeURIComponent(m[2]);
    post_list = "http://rule34.paheal.net/post/list/"+tag+"%20order=score_DESC/1";

    pahealLink.clone().attr("href", post_list).attr("name", tag)
        .insertBefore(node).data("saved_tag", tag).hover(hoverIn, hoverOut);
  }

  return true;
}

function scanTextNodes(node, tags) {
  var queue = [node];
  function scanTextNodesQueue() {
    var l, j, i, children, item, iname, hit;
    for (j = 0; j < 200 && queue.length > 0 && !hit; j++) {
      item = queue.shift();
      if (item.nodeType === 3) {
        hit = processTextNode(item, tags);
      } else {
        // uhhh this could be bad
        iname = item.nodeName.toLowerCase();
        if (iname === "script" || iname === "style") { continue; }
    ...
    if (queue.length > 0) {
      setTimeout(scanTextNodesQueue, 10);
    }
  }
  setTimeout(scanTextNodesQueue, 10);
}
...
if (enabled) {
  GM_registerMenuCommand("Rule34Everywhere - Disable", function(){
    GM_setValue("enabled", false);
    alert("Rule34Everywhere is now disabled.");
  }, "D");
  processPage();
} else {
